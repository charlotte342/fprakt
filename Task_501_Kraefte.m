...
%% kurzreichweitige Kräfte - Linked Cell
...
for h = 1:np_Teilchen
    ...
    for i = 1:n
        ...
        if coordinates(h,1) == q_O && r(i,1) == 0
            F_neu(i,:) = 24*epsilon_OO/sum(r(i,2:4).^2, 2) * sigma_OO/((sum(r(i,2:4).^2, 2))^3) * (1-2*sigma_OO^6/(sum(r(i,2:4).^2,2)^3)) * r(i,2:4);
        elseif coordinates(h,1) == q_O && r(i,1) ~= 0 || coordinates(h,1) == q_H && r(i,1) ~= 0
            F_neu(i,:) = 24*epsilon_OH/sum(r(i,2:4).^2, 2) * sigma_OH/((sum(r(i,2:4).^2, 2))^3) * (1-2*sigma_OH^6/(sum(r(i,2:4).^2,2)^3)) * r(i,2:4);
        elseif coordinates(h,1) == q_H && r(i,1) == 0
            F_neu(i,:) = 24*epsilon_HH/sum(r(i,2:4).^2, 2) * sigma_HH/((sum(r(i,2:4).^2, 2))^3) * (1-2*sigma_HH^6/(sum(r(i,2:4).^2,2)^3)) * r(i,2:4);
        end
        F_neu(isnan(F_neu)) = 0;
    end
    F_0(h,:) = sum(F_neu, 1);
